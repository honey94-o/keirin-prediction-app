...
export default async function RaceDetailPage({
  ...
  return (
    <main className="flex-1 px-4 py-4 max-w-lg mx-auto w-full">
      ...
      {alreadyRecorded ? (
        <p className="mb-4 text-center text-sm text-gray-400 border border-gray-200 rounded-lg py-2">
          この予想は記録済みです（<Link href="/history" className="underline">履歴を見る</Link>）
        </p>
      ) : (
        <form action={recordPredictionForRace} className="mb-4">
          <button
            type="submit"
            className="w-full text-center bg-white border border-[#0d5c3f] text-[#0d5c3f] rounded-lg py-2 font-semibold active:opacity-70"
          >
            この予想を記録する
          </button>
      ...
      <div className="flex flex-col gap-3">
        {scored.map((s) => (
          <div key={s.entry.entry_id} className="bg-white rounded-lg shadow-sm p-3">
