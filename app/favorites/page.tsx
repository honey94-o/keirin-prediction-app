...
export default async function FavoritesPage() {
  ...
  return (
    <main className="flex-1 px-4 py-4 max-w-lg mx-auto w-full">
      ...
      <h1 className="text-lg font-bold mb-1">お気に入り選手</h1>
      ...
      {favorites.length === 0 ? (
        ...
        <div className="flex flex-col gap-3">
          {favorites.map((racer) => {
            ...
            return (
              <section key={racer.snum} className="bg-white rounded-lg shadow-sm p-4">
                <div className="flex items-baseline justify-between mb-2">
                  <Link href={`/racers/${racer.snum}`} className="font-semibold text-gray-900">
                    {racer.name}
                  </Link>
                ...
                <div className="mb-2">
                  ...
                  {yesterdays.length === 0 ? (
                    <p className="text-xs text-gray-400">出走なし</p>
                  ) : (
                    <ul className="flex flex-col gap-1">
                      {yesterdays.map((e) => (
                        <li key={e.race.id} className="flex items-center gap-2 text-sm">
                          <Link href={`/races/${e.race.id}/bets`} className="flex-1 truncate text-gray-700">
                          ...
                          {e.finishPos != null ? (
                            ...
                            <span className="text-xs text-gray-400">結果未確定</span>
                          )}
                        </li>
                      ))}
                    </ul>
                      ...
                      {todays.map((e) => (
                        <li key={e.race.id}>
                          <Link
                            ...
                            className="flex items-center gap-2 text-sm bg-yellow-50 rounded px-2 py-1.5 active:bg-yellow-100"
                          >
                            ...
                            <span className="text-xs text-yellow-700 tabular-nums">{e.carNum}番</span>
                          </Link>
                        </li>
                      ))}
                    </ul>
