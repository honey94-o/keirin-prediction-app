...
export default async function Home() {
  ...
  if (races.length === 0) {
    return (
      <main className="flex-1 flex flex-col items-center justify-center px-6 text-center text-gray-500">
        <p>レースがまだ登録されていません。</p>
        <p className="text-sm mt-2">
          scraper/keirin_scraper.py で1レース分取得してください。
        </p>
      </main>
    );
  }
              ...
              <ul className="divide-y divide-gray-100">
                {groupRaces.map((race) => (
                  <li key={race.id}>
                    ...
                    >
                      <span className="font-medium">{race.race_no}R</span>
