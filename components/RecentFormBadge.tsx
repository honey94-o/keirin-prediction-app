...
export function RecentFormBadge({ avgFinish }: { avgFinish: number | null }) {
  ...
  if (avgFinish <= HOT_THRESHOLD) {
    return (
      <span className="inline-flex items-center gap-0.5 text-[10px] font-bold text-rose-soft bg-rose/10 px-1.5 py-0.5 rounded-full shrink-0">
        🔥好調
      </span>
    );
  }
  if (avgFinish >= COLD_THRESHOLD) {
    return (
      <span className="inline-flex items-center gap-0.5 text-[10px] font-bold text-blue bg-blue/10 px-1.5 py-0.5 rounded-full shrink-0">
        📉不調
      </span>
    );
  }
