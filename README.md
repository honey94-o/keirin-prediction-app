...
**3連単フォーメーション（上限20点）：** ◎（総合スコア1位）を1着に固定する「1頭軸流し」形式。
2着・3着候補は同じ上位M頭のプールとし、点数はM×(M-1)。上限20点を超えない最大のM（出走数が十分なら通常M=5→20点ちょうど）を自動選択する（`generateSanrentanFormation`）。出走数が少ないレースは自動的に点数が減る。
...
| 買い目提案 | `/races/[id]/bets` | 3連単フォーメーション（上限20点）・3連複ボックス候補 |
